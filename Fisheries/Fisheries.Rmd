...
#Split population column

fish.df2<-fish.df%>%
  separate(Population,
           into=c("string1", "string2","string3"),
           sep="\n")
fish.df2$string1

fish.df2<-fish.df%>%
  mutate(pop=ifelse(str_detect(Population, "[Uu]nknown"), "Unknown",
                    ifelse(str_detect(Population, "[Aa]bove"), "Above",
                           ifelse(str_detect(Population, "[Aa]bove&[Bb]elow"), "Stable",
                              ifelse(str_detect(Population, "[Bb]elow|Near"),"Below", NA)))))    
...
fish.df2<-fish.df%>%
  mutate(
    Pop_level=case_when(
      str_detect(Population, pattern = "[Uu]nknown")~"Unknown",
      str_detect(Population, pattern = "[Aa]bove")~"Above",
      is.na(Population)~NA,
      TRUE ~ "Below"
    )
  )
...
pattern="[Uu]nknown"
sum(str_detect(fish.df$Population, pattern = "([Uu]nknown)"), na.rm = TRUE)
sum(str_detect(fish.df$Population, pattern = "([Aa]bove)"), na.rm = TRUE)
sum(str_detect(fish.df$Population, pattern = "([Bb]elow)"), na.rm = TRUE)
